	...
	<script type="text/javascript">
		$(function(){
			$('#dg').datagrid({   
			    ...
			    url:'${manager}/studentquanxian/query_studentquanxian.action',
			    loadMsg:'Loading......',
			    queryParams:{studentid:''},//studentid参数，这里不需要传具体的type，因为我们要显示所有的
			    //width:300,
				...
				toolbar: [{
					...
					handler: function(){
						parent.$("#win").window({
							...
							width:350,
							height:200,
							content:'<iframe src="${manager}/send/student_save.action" frameborder="0" width="100%" height="100%"/>'
						});
					}
				},'-',{
					iconCls: 'icon-edit',
					...
					handler: function(){
						...
						if(rows.length == 0) {
							...
							parent.$("#win").window({
								...
								width:350,
								height:250,
								content:'<iframe src="${manager}/send/student_update.action" frameborder="0" width="100%" height="100%"/>'
							});
							//2. 
						}
					}
						...
						if(rows.length == 0) {
							...
							$.messager.confirm('删除的确认对话框', '您确定要删除此项吗？', function(r){
								if (r){
								    ...
								    //2. 发送ajax请求
								    $.post("${manager}/studentquanxian/deleteByIds_studentquanxian.action",{ids:ids},function(result){
								    	if(result == "true") {
			    ...
			    //title：列标题，是显示给人看的
			    columns:[[    		           
			        {field:'studentid',title:'学号',width:100, //字段type
