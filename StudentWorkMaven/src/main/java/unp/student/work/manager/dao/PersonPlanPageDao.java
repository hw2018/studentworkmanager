...
@Component
public class PersonPlanPageDao
{
	...
	public void initialize(String studentid)  //完成成员变量的初始化工作
	{
		...
		Query q = s.createQuery("select count(*) from PersonPlan where studentid=?").setParameter(0, studentid);
	...
	public List<PersonPlan> getPageResult(int curPage,String studentid)
	{
		if(curPage==0)
			return null;
		else
		{
			...
			List<PersonPlan> plans = s.createQuery("from PersonPlan where studentid=?").setParameter(0, studentid).setFirstResult((curPage - 1)*pageSize).setMaxResults(pageSize).list();
