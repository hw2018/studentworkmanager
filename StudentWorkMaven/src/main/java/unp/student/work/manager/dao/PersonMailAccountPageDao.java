...
@Component
public class PersonMailAccountPageDao
{
	...
	public PersonMailAccountPageDao()  
	{
	...
	public void initialize(String studentid)  //完成成员变量的初始化工作
	{
		...
		Query q = s.createQuery("select count(*) from PersonMailAccount where studentid=?").setParameter(0, studentid);
	...
	public List<PersonMailAccount> getPageResult(int curPage,String studentid)
	{
		if(curPage==0)
			return null;
		else
		{
			...
			List<PersonMailAccount> personMailAccounts = s.createQuery("from PersonMailAccount where studentid=?").setParameter(0, studentid).setFirstResult((curPage - 1)*pageSize).setMaxResults(pageSize).list();
