...
@Component
public class PersonDiaryPageDao
{
	...
	public void initialize(String studentid)  //完成成员变量的初始化工作
	{
		...
		Query q = s.createQuery("select count(*) from PersonDiary where studentid=?").setParameter(0, studentid);
	...
	public List<PersonDiary> getPageResult(int curPage,String studentid)
	{
		if(curPage==0)
			return null;
		else
		{
			...
			List<PersonDiary> Diaries = s.createQuery("from PersonDiary where studentid=?").setParameter(0, studentid).setFirstResult((curPage - 1)*pageSize).setMaxResults(pageSize).list();
