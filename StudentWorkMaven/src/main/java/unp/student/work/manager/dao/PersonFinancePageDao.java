...
@Component
public class PersonFinancePageDao
{
	...
	public void initialize(String studentid)  //完成成员变量的初始化工作
	{
		...
		Query q = s.createQuery("select count(*) from PersonFinance where studentid=?").setParameter(0, studentid);
	...
	public List<PersonFinance> getPageResult(int curPage,String studentid)
	{
		if(curPage==0)
			return null;
		else
		{
			...
			List<PersonFinance> finances = s.createQuery("from PersonFinance where studentid=?").setParameter(0, studentid).setFirstResult((curPage - 1)*pageSize).setMaxResults(pageSize).list();
