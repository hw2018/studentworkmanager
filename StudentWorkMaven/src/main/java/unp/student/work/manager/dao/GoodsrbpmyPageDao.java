...
@Component
public class GoodsrbpmyPageDao {
	...
	public void initialize(String studentid) // 完成成员变量的初始化工作
	{
		...
		Query q = s.createQuery("select count(*) from Goodsrbpmy where studentid=?").setParameter(0, studentid);
	...
	public List<Goodsrbpmy> getPageResult(int curPage, String studentid) {
		if (curPage == 0)
			return null;
		else {
			Session s = sessionFactory.getCurrentSession();
			@SuppressWarnings("unchecked") // 告诉编译器忽略指定的警告，如果没有这一句，编译器会警告funds是unchecked的
			List<Goodsrbpmy> favorites = s.createQuery("from Goodsrbpmy where studentid=?").setParameter(0, studentid)
