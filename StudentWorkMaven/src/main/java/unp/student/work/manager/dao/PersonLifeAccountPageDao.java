...
@Component
public class PersonLifeAccountPageDao
{
	...
	public PersonLifeAccountPageDao()  
	{
	...
	public void initialize(String studentid)  //完成成员变量的初始化工作
	{
		...
		Query q = s.createQuery("select count(*) from PersonLifeAccount where studentid=?").setParameter(0, studentid);
	...
	public List<PersonLifeAccount> getPageResult(int curPage,String studentid)
	{
		if(curPage==0)
			return null;
		else
		{
			...
			List<PersonLifeAccount> personLifeAccounts = s.createQuery("from PersonLifeAccount where studentid=?").setParameter(0, studentid).setFirstResult((curPage - 1)*pageSize).setMaxResults(pageSize).list();
			
			return personLifeAccounts;
		}
