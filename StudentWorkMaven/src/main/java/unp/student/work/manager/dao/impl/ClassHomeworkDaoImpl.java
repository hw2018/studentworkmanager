package unp.student.work.manager.dao.impl;

import java.util.List;

import org.springframework.stereotype.Repository;

import unp.student.work.manager.dao.ClassHomeworkDao;
import unp.student.work.manager.domain.ClassHomework;

@Repository("classHomeworkDao")
public class ClassHomeworkDaoImpl extends BaseDaoImpl<ClassHomework> implements ClassHomeworkDao {

	public List findByPage(int pageNo, int pageSize) {
		// TODO Auto-generated method stub
		return findByPage("from ClassHomework", pageNo, pageSize);
	}

	public List findById(int id, int pageSize){

		return findById( id, pageSize);
	}

}
