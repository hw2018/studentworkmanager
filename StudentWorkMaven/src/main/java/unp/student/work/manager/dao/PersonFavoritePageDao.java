...
@Component
public class PersonFavoritePageDao
{
	...
	public void initialize(String studentid)  //完成成员变量的初始化工作
	{
		...
		Query q = s.createQuery("select count(*) from PersonFavorite where studentid=?").setParameter(0, studentid);
	...
	public List<PersonFavorite> getPageResult(int curPage,String studentid)
	{
		if(curPage==0)
			return null;
		else
		{
			...
			List<PersonFavorite> favorites = s.createQuery("from PersonFavorite where studentid=?").setParameter(0, studentid).setFirstResult((curPage - 1)*pageSize).setMaxResults(pageSize).list();
